...
def validate_bind_host(host: str) -> str:
    ...
    # 0.0.0.0 is the explicit LAN-server mode; it listens on local interfaces
    # while still rejecting public, multicast, and link-local bind addresses.
    if address.is_multicast or address.is_link_local or address.is_reserved or not address.is_unspecified and not (address.is_loopback or address.is_private):
...
def serve(project_root: Path, host: str = "0.0.0.0", port: int = 8765) -> DistributionHttpServer:
    server = DistributionHttpServer((validate_bind_host(host), port), DistributionService(project_root))
    server.discovery_thread = threading.Thread(target=_discovery_loop, args=(server,), daemon=True)
    server.discovery_thread.start()
    return server


def _discovery_loop(server: DistributionHttpServer) -> None:
    payload = json.dumps({"service": "datatang-distribution", "protocol": 1, "port": server.server_port}).encode("utf-8")
    try:
    ...
    with sock:
        sock.settimeout(0.5)
        while not server.discovery_stop.is_set():
            try:
                data, address = sock.recvfrom(128)
            except socket.timeout:
                continue
            if data.strip() == b"DATATANG_DISCOVER_V1":
