...
工具内置左手单键和数字键盘两套快捷操作，并支持按 `F1` 自定义：

- `A / D` 或数字键盘 `4 / 6`：上一组、下一组。
- `Space` 或数字键盘 `5`：质检通过并进入下一组。
- `X` 或数字键盘 `0`：进入返修备注；`Enter` 提交，`Shift+Enter` 换行。
- `Q / E` 或数字键盘 `1 / 3`：本地 AI 检测、在线深度复核。
- `1 / 2 / 3`：采纳 AI 标签、采纳 AI 备注、清空 AI 结果。
- `F / V / Tab / + / -`：图片适应、1:1、切换焦点和缩放。
- `Z / C / Esc / F1 / F2`：撤销、刷新、退出输入、快捷键设置和 AI 设置。

在中文、英文指令或普通输入框中编辑时，单键快捷键会自动停用，避免影响正常输入；删除数据组仍执行原有两次确认。详细说明见 `docs/SHORTCUTS.md`。
